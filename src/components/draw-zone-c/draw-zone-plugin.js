    ...
    function $singleRect_moveDrawLine($event) {
        // console.log($event,this);
        let x = $event.offsetX, y = $event.offsetY, w = $event.srcElement.clientWidth, h = $event.srcElement.clientHeight;
        let ctx = $event.currentTarget.lastElementChild.getContext("2d");
        ctx.clearRect(0, 0, w, h);
        if(this.draw.vertex.length <= 3){
            ctx.beginPath();
            ctx.lineWidth = 2;
            ctx.setLineDash([2,5]);
            ctx.strokeStyle = "#000";
        ...
        '$single-rect': {
            ...
            'create': {
                ...
                'click' ($event) {
                    ...
                    $singleRect_moveDrawLine.call(this, $event);
                },
